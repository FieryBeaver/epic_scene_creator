...
export function inspConn(c){
  ...
  return `<div class="ihead">
      ...
      <div class="s">${T('conn.external')}</div>
    </div>
    <div class="ipad">
      <fieldset><legend>${T('conn.ends')}</legend>
        ...
        <button class="btn sm" data-swap="${id}" style="margin-top:7px">${T('conn.swap')}</button>
        ${blockNote(c)}
      </fieldset>

      <fieldset><legend>${T('conn.props')}</legend>
        <label class="f"><span>${T('conn.name')}</span>
          <input type="text" data-path="c:${id}:name" value="${esc(c.name)}"></label>
        <div class="grid2">
          <label class="f"><span>${T('conn.type')}</span>
            <select data-path="c:${id}:dir">
              <option value="two"${c.dir === 'two' ? ' selected' : ''}>${T('conn.twoWay')}</option>
              <option value="one"${c.dir === 'one' ? ' selected' : ''}>${T('conn.oneWay')}</option>
            </select></label>
          <label class="f"><span>${T('conn.minutes')}</span>
            <input type="number" min="0" data-num="1" data-path="c:${id}:minutes" value="${esc(c.minutes)}"></label>
        </div>
        <div class="grid2">
          <label class="f"><span>${T('conn.exits', { name: a ? a.name : '?' })}</span>
            <select data-path="c:${id}:fromSide">${sideOptions(c.fromSide)}</select></label>
          <label class="f"><span>${T('conn.enters', { name: b ? b.name : '?' })}</span>
            <select data-path="c:${id}:toSide">${sideOptions(c.toSide)}</select></label>
        </div>
        <label class="f"><span>${T('conn.how')}</span>
          <textarea data-path="c:${id}:desc">${esc(c.desc)}</textarea></label>
        <label class="tgl">
          <input type="checkbox" data-path="c:${id}:open" ${c.open !== false ? 'checked' : ''}> ${T('conn.open')}</label>
        <div class="row" style="margin-top:8px">
          <select class="btn sm" data-tpl="conn" data-id="${id}" style="width:auto">
            <option value="">${T('scene.fromTemplate')}</option>
            ${TPL_CONN.map((tpl, i) => `<option value="${i}">${esc(tplName('conn', tpl))}</option>`).join('')}
          </select>
          <button class="x" data-del-conn="${id}">${T('conn.delete')}</button>
        </div>
      </fieldset>
...
function blockNote(c){
  ...
  return `<div style="font-size:11px;color:var(--sky);margin-top:7px">
    ⛔ ${T('conn.blockedBy', { name: covered.block.nm })}`
    + `${covered.block.done ? T('room.solvedSuffix') : ''}
...
function counters(c, id){
  let h = `<fieldset><legend>${T('counter.connLegend')}</legend>`;
  if (!c.counters.length){
    h += `<div class="empty">${T('counter.connNone')}</div>`;
  }
  ...
  return h + `<button class="btn sm" data-add="conncounter" data-id="${id}">${T('counter.connAdd')}</button></fieldset>`;
}

function tokens(toks){
  let h = `<fieldset><legend>${T('conn.tokensLegend', { n: toks.length })}</legend>`;
  if (!toks.length) h += `<div class="empty">${T('conn.tokensNone')}</div>`;
  toks.forEach(tok => {
